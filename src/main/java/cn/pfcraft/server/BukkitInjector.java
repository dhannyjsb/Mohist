    ...
    public static void injectItemBukkitMaterials()
    {
        for (Map.Entry<ResourceLocation, Item> entry : ForgeRegistries.ITEMS.getEntries()) {
            ...
            if(!key.getResourceDomain().equals("minecraft")) {
                ...
                if (material != null) {
                   Mohist.LOGGER.debug("Injected new Forge item material %s with ID %d.", material.name(), material.getId());
                } else {
                    Mohist.LOGGER.debug("Inject item failure %s with ID %d.", materialName, Item.getIdFromItem(item));
                }
            }
        }
    }
    ...
    public static void injectBlockBukkitMaterials()
    {
        ...
        for (Map.Entry<ResourceLocation, Block> entry : ForgeRegistries.BLOCKS.getEntries()) {
            ...
            if(!key.getResourceDomain().equals("minecraft")) {
                ...
                if (material != null) {
                    Mohist.LOGGER.debug("Injected new Forge block material %s with ID %d.", material.name(), material.getId());
                } else {
                    Mohist.LOGGER.debug("Inject block failure %s with ID %d.", materialName, Block.getIdFromBlock(block));
                }
            }
        }
    }
